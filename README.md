...
このMODはセマンティックバージョニング(`MAJOR.MINOR.PATCH`)に従います。GitHub Issue #25(「軽微な変更でもパッチではなくマイナーバージョンを上げるのはどうなのか」)を受けて、v0.25.1までの運用を見直し、以降は以下の基準で判断します。

- **PATCH (x.y.Z)**: 既存機能のバグ修正、ビルド修正、テクスチャー/バランスの微調整、ドキュメント更新など、新しいプレイヤー向けコンテンツを追加しない変更。
- **MINOR (x.Y.z)**: 新しいブロック・アイテム・MOB・機能・ディメンションギミックなど、後方互換な新規コンテンツの追加。
- **MAJOR (X.y.z)**: 既存のワールドデータ/セーブとの互換性を壊す変更、または大規模な再設計(v1.0.0に到達するまでは互いに未定)。

判断に迷う場合は「プレイヤーが新しく触れる要素が増えたか」を基準にします。増えていなければPATCH、増えていればMINORです。過去のバージョン(v0.22.0〜v0.25.0など)は、この基準に照らすと本来PATCHにすべきだった変更でMINORを上げてしまっているものが含まれており、当時の判断が甘かったことを認めます。今後のリリースはこの方針に従います。
