...
最終更新: 2026-09-23(定期実行セッション、v0.56.0リリース: 新規アイテム「プリズミウムの解毒石」(Prismium Antivenom Charm)を追加。羽石/火除け石/活力石/磁石の護符/護盾石/耐寒石に続く7つ目の完全パッシブcharmで、毒(minecraft:magicダメージタイプ、バニラに専用の毒ダメージタイプが無いため)・衰弱(minecraft:witherダメージタイプ)を両方対象に50%軽減する。これで代表的な環境ダメージタイプ(落下・炎・爆発・凍結・毒/衰弱)が出揃った。クラフトレシピはクモの目x2+ウィザーローズx1+プリズミウムの欠片x1のシェイプレス(パターン不整合の再発防止のため意図的にshapeless、1個産出)。テクスチャーは既存charm群と同じ「石+対角線モチーフ+プリズミウム宝石」構図で、モチーフに黄緑色の毒々しい雫を新規デザイン、Pillowで生成し16倍拡大プレビューで自己レビュー済み(サンプルの意図した「独立した泡」演出は輪郭線で本体と視覚的に繋がって見えるが、破綻はしていないと判断)。Issue #15/#21は個別ページで再確認したが新規コメント無し(11セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし、新規issue番号の出現も無し。CIのbuild-and-notify(実装コミットae35b72→run 364、バージョンbumpコミット45906cc→run 365)・Releaseワークフロー(タグv0.56.0→run 69)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=45906cc...`かつ新規レシピのパースエラーが無いことを確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.56.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
