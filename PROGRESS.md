...
最終更新: 2026-09-18(定期実行セッション、v0.51.0リリース: 新規装飾ブロック「模様入りプリズミウム合金ブロック」(Chiseled Prismium Alloy Block)・「模様入り蒼白のプリズミウムブロック」(Chiseled Pale Prismium Block)を追加。プリズミウムブロック・プリズミウムコアには既にある「模様入り(Chiseled)」装飾バリエーションが、スラブ/塀/階段は揃っているのに無かった2ブロック(プリズミウム合金ブロック・蒼白のプリズミウムブロック)の穴を埋めた。新しいJavaクラス・メカニクスは導入せず、既存2種の模様入りブロックと全く同じ`new Block(...)`パターン(各ベースブロックと同じstrength/sound/lightLevel)。テクスチャーはPythonで新規生成(既存の模様入りブロックの「額縁+中央の菱形紋章」構図を踏襲しつつ、合金版は鋼青色METAL_*パレット+マゼンタアクセント、蒼白版はPALE_*氷パレットでそれぞれ再配色)、24倍チェッカーボードプレビューで自己レビュー済み(不透明・視認性とも問題なし)。クラフトレシピは各ブロックのハーフブロックx2を使うshaped(模様入りプリズミウムブロック/コアと同じ方式)。mineable/pickaxeタグに追加、needs_iron_tool/needs_diamond_toolには未追加(TODO16で触れているPRISMIUM_ALLOY_BLOCK/PALE_PRISMIUM_BLOCK自体の既存の非対称性にそのまま合わせた判断)。Issue #15/#21は個別ページで再確認したが新規コメント無し(6セッション連続で変化無し)、Issues一覧のOpenバッジも(2)のまま変化なし。CIのbuild-and-notify(コード変更コミットd3785ee→run 348、バージョンbumpコミット855387b→run 349)・Releaseワークフロー(run 64)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=855387b...`を確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.51.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
