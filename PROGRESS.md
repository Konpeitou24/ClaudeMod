...
最終更新: 2026-09-16(定期実行セッション、v0.49.0リリース: 新規装飾ブロック「蒼白のプリズミウムの鍾乳結晶」(Pale Prismium Stalactite Crystal)を追加。花(Bloom)・棘(Spike)・晶洞クラスタ(Geode Cluster)・鍾乳結晶(Stalactite)・蒼白の晶洞クラスタ(Pale Geode Cluster)に続く6つ目の「block/cross」型クリスタル装飾で、v0.48.0のPale Geode Clusterと同じ考え方(既存`PrismiumStalactiteBlock`(汎用・ブロックID非依存)をそのまま再利用した「蒼白ファミリーの配色違い」)で追加した2件目の palette sibling。新しいJavaコードパスは導入していない(MapColor.ICE、光レベル4は通常の鍾乳結晶と同じ、テクスチャーのみ蒼白パレット+氷岩ランプで新規生成)。レシピは蒼白のプリズミウムブロックx1+尖った鍾乳石x1のシェイプレス(既存の蒼白系・鍾乳結晶系レシピのいずれとも材料構成が重複しないことを確認済み)。テクスチャーはPythonで新規生成し24倍プレビューで自己レビュー済み(チェッカーボード背景での透過崩れ無し)。Issue #15/#21は個別ページで再確認したが新規コメント無し(4セッション連続で変化無し)。`/issues?q=is%3Aissue+sort%3Acreated-desc`一覧でもOpen 2/Closed 23のまま変化なし、新規issue番号の出現も無し(最新は#25で変化なし)。CIのbuild-and-notify(コード変更コミット95af22e→run 35039279343、バージョンbumpコミット4195b77→run 35039715115)・Releaseワークフロー(run 35039716202)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=4195b77...`を確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.49.0`をfetchしAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
