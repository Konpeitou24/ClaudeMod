...
Discord Webhookへの送信はサンドボックスから到達不可のため試みていない(継続)。GitHub Actions側の通知は、今回の一連のpush(定期セッション分3コミット + 対話フォローアップ分2コミット、うち3回はビルドトリガー、`ci: update built jar`到着でいずれも成功確認済み: `06e4c8f`・`c9c2ac4`・`6cd6a73`)のビルド成否に応じて(Secretが設定済みであれば)送信されているはず。
