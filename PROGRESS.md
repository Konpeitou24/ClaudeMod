...
3. **【新規・対話セッション、2026-08-26】音声(サウンド)方針: バニラ音源の流用とPython生成のオリジナル音源を、場面に応じて両方うまく使い分けること。** きっかけは直対セッションでPrismium Portalに初のオリジナル音源(起動音・消滅音、`scripts/sounds/gen_portal_sounds.py`でnumpy+ffmpegにより合成)を追加したこと。ユーザーから「今後もバニラの音声とPythonで作成した音声をうまく活用してください」と直接の恒久指示があった。
   - 具体的な使い分けの目安(この指示を受けての解釈、今後のセッションで磨き込んでよい): MOBの鳴き声・被弾音・死亡音のように「生き物らしい呼吸感やランダム性」が重要な音は、無理に一から合成せずバニラ流用が自然な場合が多い(現状Wraith/Sentinel/Drifter/Deep Wraithは全てバニラ音源の流用、Vex/Illusioner/Glow Squid/Guardian等)。一方、このMOD独自のギミック・ブロック(ポータルの起動/消滅のような「この世界観だけの現象」)には、既存のバニラ音源に適切な代替が無いことが多く、Python合成によるオリジナル音源の方が向いている。
   - 音声合成の技術的な確立済み手法: `numpy`でサイン波スイープ・チャイム(倍音スタック)・フィルタ済みノイズ・クラックル(ノイズバースト)等を合成し、標準ライブラリの`wave`モジュールでWAV書き出し、`ffmpeg -c:a libvorbis`でMinecraft必須の`.ogg`形式に変換する(`scripts/sounds/gen_portal_sounds.py`参照)。登録は`DeferredRegister<SoundEvent>`(`ForgeRegistries.SOUND_EVENTS`)+`SoundEvent.createVariableRangeEvent`、`assets/<modid>/sounds.json`でイベント名とogg参照・字幕キーを紐付ける。
   - **自己レビューの限界(正直な記録)**: このサンドボックス環境には音声出力デバイスが無く、実際に「聞く」ことができない。テクスチャーの目視レビューに相当する代替手段として、生成スクリプトでピーク振幅・RMS・長さ・NaNの有無を出力し、`ffprobe`でogg自体が正しくデコードできる有効なVorbisストリームであることを確認している。ただし「音として実際に良い/世界観に合っているか」は毎回ユーザー側でのプレイ後フィードバック待ちになる。次回以降、新しいオリジナル音源を作る際もこの技術検証パターンを踏襲すること。
