...
最終更新: 2026-09-21(定期実行セッション、v0.54.0リリース: 新規ブロック「プリズミウムの壁灯」(Prismium Wall Lamp)を追加。このMOD初の`BlockStateProperties.HORIZONTAL_FACING`を使った壁掛け専用ブロックで、床/天井には設置できず横の壁面にのみ設置可能(既存PrismiumLanternBlockの支持判定パターンを踏襲)。厚さ2pxの平板シェイプを向きごとに4種キャッシュし、支えを失うと自動で外れる。ブロックステートはvanilla furnace.jsonと同じnorth=0/east=90/south=180/west=270のY回転マッピングを採用。見た目は暗い金属背板+四隅リベット+中央の発光菱形プリズミウム宝石、Pillowで新規生成し16倍拡大プレビューで自己レビュー済み。光レベル13。レシピはグロウストーンダスト+プリズミウムの欠片+鉄塊の縦一列shaped(2個産出)。**このセッションでもう一つ重要な発見・修正あり**: ビルド検証ログを精査した結果、前回v0.53.0で追加した「プリズミウムの耐寒石」のクラフトレシピJSONが`Key defines symbols that aren't used in pattern: [S]`という例外でレシピ自体が一切ロードされておらず、公開時点から実質クラフト不可能だったことが判明(原因は氷塊x4+粉雪バケツx1+欠片x1の6アイテムを十字型5マスに収めようとした設計そのものの破綻)。3x3パターンに組み直して修正し、CI検証で当該パースエラーの解消を確認済み(詳細は「3. 問題点」参照)。Issue #15/#21は個別ページで再確認したが新規コメント無し(9セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし。CIのbuild-and-notify(Wall Lampコミットc4c74ec→run 357、Frostguardレシピ修正コミットe28e399→run 358、バージョンbumpコミット42f4ddd→run 359)・Releaseワークフロー(run 67)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=42f4ddd...`かつ耐寒石のパースエラーが消えたことを確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.54.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
