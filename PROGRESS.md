...
最終更新: 2026-09-03(定期実行セッション、v0.40.0リリース: TODO11「GameTestフレームワークで、ContainerDataの同期を自動検証する仕組みを追加する」に対応。GitHub Issue #15の具体的な2件の不具合報告(「発電機の隣にセルを置くと2倍量貯蓄される」「ケーブルが6方向にしか電力を送れない」)に対応する回帰テストを`ClaudeModGameTests`(`com.claudemod.gametest`)として新規実装し、CI既存の`runGameTestServer`スモークテスト上で実際に実行・成功を確認した(このサンドボックスで「実際にゲームを動かして」検証できた数少ない事例。`server_validation_tail.log`で`All 2 required tests passed :)`を直接確認済み)。実装中に新たな教訓を発見: Forgeの`@GameTestHolder`アノテーションはForgeのバージョンによって引数の形が異なり(新しいドキュメント/mappings.devのページには`namespace()`属性があったが、このMODが固定しているForge 47.4.0(1.20.1ブランチの実ソース)には`value()`一つしか無く、`namespace = ...`と書いたら`cannot find symbol`でビルド失敗した)。1回ビルド失敗した後、GitHub生ソース(`raw.githubusercontent.com/MinecraftForge/MinecraftForge/1.20.1/...`)で実際のクラス定義を確認して修正、build-and-notify #302でStatus Success・GameTest 2件成功を確認、Release #49(v0.40.0)もStatus Success・Assets 3(jar付き)を確認済み。詳細はTODO・問題点・約束や決まり事を参照。GitHub Issueは`is:issue`で再確認、#15・#21の2件のみで新規issueなし。#15の最新コメントは引き続き「動力系のすべてのUIが全く機能していません」で、今回追加したテストが直接検証するのはサーバー側のエネルギー移動・二重計上防止のみである点に注意(クライアント側GUI描画の検証はGameTestでは不可能、TODO6/TODO7として引き続き未解決))
...
11. **【v0.40.0で対応・CI上で実際にテスト成功を確認済み】** GameTestフレームワークで、Issue #15の2件の具体的な報告(セルの二重貯蓄・ケーブルの6方向制限)への回帰テストと、発電機ContainerDataのサーバー側同期テストを追加した(`com.claudemod.gametest.ClaudeModGameTests`)。CIの`runGameTestServer`上で`All 2 required tests passed`を実際に確認済み(builds/last_datapack_validation_tail.log参照)。**次にやるべき拡張**: (a) 他の機械(Pylon/Restorer/Wardstone/Pulverizer/Smelter/Compressor)のContainerData同期テストへの横展開、(b) より複雑なケーブル網(分岐・ループ・複数発電機/複数消費ブロック)での保存則テスト(TODO6の「二段階の挙動」報告の自動再現を試みる価値がある)、(c) MenuのquickMoveStack(shift-click)のGameTestでの検証(GameTestは実プレイヤーのクリック操作は再現できないが、`AbstractContainerMenu#clicked`をサーバー側から直接呼び出すことで擬似的に検証できる可能性がある、要調査)。**このテストが検証しないもの**: クライアント側のGUI描画・ContainerDataのクライアント受信(session #84で修正したバグの再発)はGameTestServerにクライアントが接続されないため検証不可能(実機確認が引き続き必要、TODO6/TODO7参照)。
