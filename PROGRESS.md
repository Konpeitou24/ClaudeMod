...
最終更新: 2026-09-20(定期実行セッション、v0.53.0リリース: 新規アイテム「プリズミウムの耐寒石」(Prismium Frostguard Charm)を追加。羽石(Featherstone)/火除け石(Emberguard)/活力石(Vitastone)/磁石の護符(Magnet Charm)/護盾石(Aegis Charm)に続く6つ目の完全パッシブ系charm(装備不要、インベントリに持っているだけで効果、Curios charmスロットにも対応)。効果は粉雪による凍結(フロストバイト)ダメージ50%軽減。凍結ダメージはFire Protection/Blast Protectionに相当するバニラの軽減エンチャントが存在しない、このMOD初のダメージ系charmとなる。新しいJava APIとして`DamageTypes.FREEZE`(ResourceKey<DamageType>)と`DamageSource#is(ResourceKey<DamageType>)`をmappings.devで実在確認してから使用(未確認API使用ルールを遵守)。テクスチャーは既存4種と同じ「石+対角線モチーフ+中央のプリズミウム宝石」構図を踏襲しつつ、モチーフをジャギーな氷柱(アイシクル)にし、未使用だった水色(シアンブルー)系配色でPillowにより新規生成、プレビューで自己レビュー済み。クラフトレシピは氷塊x4+粉雪バケツx1+プリズミウムの欠片x1の十字配置shaped。Issue #15/#21は個別ページで再確認したが新規コメント無し(8セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし。CIのbuild-and-notify(コード変更コミットf258b58→run 354、バージョンbumpコミットceca0f3→run 355)・Releaseワークフロー(run 66)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=ceca0f3...`を確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.53.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
...
- **【v0.53.0で新規・実機未検証】** プリズミウムの耐寒石(Prismium Frostguard Charm、TODO25参照)。CIビルド・データパック検証は成功済みだが、実機でのクラフト・凍結ダメージ軽減の体感・Curios連携・見た目は未検証。
