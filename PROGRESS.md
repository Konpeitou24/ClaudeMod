...
最終更新: 2026-08-31(直接チャットセッション、v0.33.0リリース: プリズミウム・クロノフレイムにGUI実装、Clock所持要件を撤廃)
...
1. **【最優先・新規、2026-08-31直接チャットでの依頼】プリズミウム・クロノフレイムに操作GUI(進める/戻すボタン)を実装し、時計を持たなくても右クリックで画面が開くようにした(旧: issue #16対応で「時計を持っていないと反応しない」実装だったが、こんぺいとう氏本人からチャットで直接「煩雑で使い物にならない、UIを実装してほしい」との強い指摘があり、即座に対応)。次回こんぺいとう氏に実機確認を依頼すること: (a) 右クリックで画面が開くか(何も持っていなくても)、(b) 「進める」「戻す」ボタンで実際に時刻が変わるか、(c) クールダウン中に両ボタンが灰色になり残り秒数が表示されるか。このMOD初のBlockEntityを持たないメニュー(SimpleMenuProviderベース)かつ初のclickMenuButton使用なので、Menu/Screen自体の実装ミスがあれば他のGUIとは違う種類の不具合(画面が開かない、ボタンが反応しない等)になりうる点に注意。**
2. **【最優先・新規】v0.31.2で「全GUIブロックの画面固まりバグ」を修正した(問題点参照)。修正内容はコードレビューとForge公式ドキュメントの記述に基づく(このサンドボックスでは実機確認不可)。次回、こんぺいとう氏に粉砕機/精錬機/圧縮機だけでなくPrismium Cell/Generator/Pylon/Restorer/WardstoneのGUIも含めて実機確認を依頼すること。直っていなければ最優先で再調査(その場合、今回の仮説=クライアント側Menuコンストラクタが本物のContainerDataを使い回していた、が的外れだった可能性が高いので振り出しに戻ること)。**
3. 【新規・優先度を上げた】GameTestフレームワーク(このMODが既にCuriosAPI検証で使っている`runGameTestServer`基盤)で、ContainerDataの同期(クライアント側`set()`が実際に値を反映するか)を自動検証する仕組みを追加する。今回のバグが8ブロック全てに共通する設計ミスだったにもかかわらずCIビルド成功だけでは検知できなかった教訓を踏まえ、再発防止として優先度を上げた。
4. プリズミウム・クローラー(v0.31.0で追加した5体目のMOB)が実機で意図通り動作しているか(見た目・自然スポーン・鳴き声)、こんぺいとう氏に確認を依頼する。
5. Issue #18対応(Curios統合)の残りの実機確認。**【2026-08-30、こんぺいとう氏が実機確認済み】Guardian CharmはCurios対応済みで、右クリックでcharmスロットに装備できることを確認済み。** 残る未確認は、Featherstone/Emberguard/Vitastone/Magnet Charmの4アイテムのCurios GUIでのスロット表示・ドラッグ&ドロップ・右クリック装備、および全5アイテム共通で「装備中に効果が実際に発動するか」。これらの確認が取れ次第issue #18をクローズする。
...
10. 【2026-08-31、新規】v0.32.0で追加したプリズミウム・コンペンディウム(issue #7対応のガイドブック)について、次回こんぺいとう氏に実機確認を依頼すること。確認したい点: (a) 初回ワールド参加時に自動で1冊インベントリに入るか、(b) 本を開いた際に11ページ全て正常に表示され、日本語ページが本のページ内に収まりきっているか(はみ出していないか)、(c) クリエイティブタブにも表示されているか。
11. コンペンディウムを紛失した場合の再入手手段(クラフトレシピ等)がまだ無い。バニラの本+プリズミウムの欠片のような簡単なレシピで再発行できるようにする(専用NBTを持つカスタムレシピが必要、vanillaのBookCloningRecipeのような特殊レシピクラスの実装が要る)。
12. コンペンディウムの内容は現状11ページの概要のみ。今後、各エネルギー機械の詳細な配線図解や、Prism Realmのダンジョン/ボス実装(TODO7)が進んだらその案内ページを追加するなど、実装が進むたびに内容を拡充していく。
...
- issue #7(MODの説明不足)は、v0.32.0のプリズミウム・コンペンディウムで大きく前進したが、完全クローズにはまだ早いと判断(実機未検証、かつ「ゲーム内で知ることができない」という報告の核心にはまだ再入手手段が無い等の穴がある)。実機確認とTODO10(再入手レシピ)が済んでからクローズを検討する。
