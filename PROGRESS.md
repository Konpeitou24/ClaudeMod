...
最終更新: 2026-09-01(定期実行セッション#2、v0.38.0リリース: TODO9「Prism Realmにまず陸地(平原などの基本地形)を追加する」に対応。PrismiumLandFeatureを新規実装し、既存のPrismiumSeafloorFeature/PrismiumStoneTransitionFeatureと同じ手法(flat generatorのためraw_generationステップのFeatureで事後上書き)で、低周波FractalNoiseによる陸地/海の判定、海面上最大6ブロックの高低差+細かい起伏、表層3ブロックがprismium_soil・それ以外prismium_stoneの2層構成を実装。build-and-notify #295・Release #47ともActionsページで実際にStatus確認済み(push後にIn progressの間は待機し、完了後に確認する運用を徹底)、リリースページに本文・jar付きAssetsがあることも確認済み。GitHub Issueは`is:issue`で全件確認(#22〜#25は既存解決済みでCLOSED、**Issue #17(羽石)がこんぺいとう氏により本日CLOSED(stateReason: COMPLETED)を確認** - v0.36.0のHUDパネル方式で解決した模様、TODOから除去。#15・#21は引き続きOPENで新規コメント・新規issueなし)。詳細はTODO・問題点参照)
...
10. MOBのカテゴリ拡充を検討する(現状「戦闘」「水中非戦闘」「地上アンビエント」の3種類、飛行アンビエント・使い魔的MOB等の余地あり)。
