...
### すぐやるべきこと
1. **【最優先、恒例】まずセッション開始時に`git fetch origin main`し、直前セッション最終コミットの直後に`ci: update built jar`コミットが付いているか確認する。** セッション#16終了時点では、`b87d192`(Prismium Locator追加)の直後に`8cc26c7`が付いており、ビルド成功を確認済み。`api.github.com`は今回も不可、Actionsページのキャッシュ問題(§2-7)も今回再現したので、`git fetch`によるjarコミット確認を主たる手段として使い続けること。
2. **【新規、低優先度だが1行で済む】§4-26の非推奨API警告(`ResourceLocation`/`FMLJavaModLoadingContext`)は、1.20.1では対応不能と判明済み(§3O-2、§4-27)。今後このタスクを「やるべきこと」として再度挙げないこと** - MC/Forgeのメジャーバージョンを上げる話が出た時にだけ再検討すればよい。
3. 【継続、優先度高】Prismium Locator(§3O-3、session 16)は実際に使ってみてどう感じるか一切未検証(§4-28)。探索半径・クールダウン・デッドゾーンの数値調整、あるいは「行動バーメッセージだけで十分か、もっと視覚的なフィードバック(パーティクルの誘導線など)が欲しくなるか」はプレイフィードバック待ち。
...
5. 【継続、優先度高】Prismium Wraith(§3K、session 12)は実際にスポーンするか・AIが正常に動くか・テクスチャーが3Dモデルに正しく貼られるかは一切未検証のまま(§4-20)。まだ「1体だけ」の状態でもあるので、(a) 2体目の追加、(b) 既存Wraithへの独自AI Goal追加、のどちらかが次の一歩として自然。
6. 【継続、優先度高】Prismium Cable・Generator・Cellを組み合わせた「発電→送電→蓄電」の3点セットは、実際にゲーム内で並べて動作確認したセッションはまだゼロ(session 10から持ち越し、6セッション経過)。
7. **ロードマップ§1の4本柱のうち、Prism Realmは専用地形・専用バイオーム・専用鉱石・本格ポータルブロックがまだ無い**(§4-24参照)。次にディメンション方面を触るなら、(a) 本格的なポータルブロック+フレーム検知(§3M-2で意図的に先送りした部分)、(b) 専用バイオーム・専用地表ブロックの追加、のどちらかが自然な発展先。ただし上記4の実プレイ検証(基本的な行き来ができるか)を優先すべき。
8. push前に必ず `git fetch origin main` → 差分があれば `git rebase origin/main`(§2-5)。session 16では他セッションとの並行は検知しなかったが、毎回確認すること。
9. **【新規、環境まわりの実務上の注意】このサンドボックスの`/tmp`は、他の並行セッションが所有するファイル/ディレクトリが残っていることがあり、固定パス(例: `/tmp/work/ClaudeMod`)を使うと`Permission denied`(所有者が`nobody`など別ユーザー)や`fatal: could not lock config file`で失敗することがある(session 16で複数回遭遇)。`git clone`は`/tmp/cm_$$_$RANDOM`のような一意なパスに対して行うこと。またマウントされた出力フォルダ(Windows側の`outputs`)は`git`のロックファイル操作(unlink等)に対応していないことがあるため、gitリポジトリの作業ディレクトリとしては使わないこと(session 16で最初に遭遇、`/tmp`配下の一意パスに切り替えて解決)。
10. issueのbodyを取得する際は`github.com/<owner>/<repo>/issues/<番号>?nocache=<ts>`のHTMLから`"body":"..."`のJSON文字列を`python3`の文字列探索で抜き出す方法が引き続き有効。

### 議論したい論点・改善案
- **プレイテストの手段が無い問題**: 依然として最大のボトルネック。ユーザー側でビルド済みjar(`builds/ClaudeMod-latest.jar`、session 16のビルド成功分)を時々プレイし、フィードバック(できればGitHub Issueの形で)を残していただけると、次回以降のセッションがそれを最優先で拾える。特にPrism Realm/Prismium Rift Shard、Prismium Wraith、Prismium Locatorは「コンパイルは通るが本当に動くか一切未確認」という段階のものが積み上がっているので、フィードバックの価値が大きい。
...
- **新しいシンボル未検証バグの教訓(§3N-3、session 16でも実践)**: 「このMOD内で既に動いている実例があるAPIパターン」は目視レビューでも比較的安全に裏取りできるが、「このセッションで初めて(かつ唯一)使う定数・enum値」は要注意。session 16ではこの教訓を2回活かした: (1) Prismium Locatorの新規SoundEvents定数を使う前にWeb検索で実在確認、(2) §4-26の非推奨API「対応」に安易に着手せず、まず置き換え先メソッドの実在を確認してから(結果的に存在しないと分かり)対応を見送った。後者は「検証してから直す」がコード変更そのものを回避した好例として記録しておく。
- **探知アイテムの発展余地**: Prismium Locator(session 16)はメッセージ表示のみの最小実装。将来的に(a) バニラコンパスのようなアイテムモデルpredicateで針を実際に回転させる、(b) Prismium Wraithなど鉱石以外の対象も検知できるようにする、(c) Prism Realm内での探知(まだ鉱石がPrism Realmに実際に生成されるか自体が未検証、§4-24)、などの拡張が考えられる。
...
### コミット/プッシュ状況
このセッションの変更は1コミット: `b87d192`(Prismium Locatorの実装・テクスチャー・レシピ・lang一式)。他セッションとの並行は検知せず(push前の`git fetch`で差分無し)、素の`git push origin main`が一度で成功(プロキシ回避策は不要だった)。push後、`git fetch`のポーリングで約6分半後に`ci: update built jar [skip ci]`コミット(`8cc26c7`)の到着を確認し、本物のビルド成功を確定させた。§4-26の非推奨API対応は調査の結果「対応不要」と判断し、コード変更は行わなかった(§3O-2)。issue #1・#2ともOpenのまま変化無し、新規Issueも無し(§3O-1)。

### 通知状況
Discord Webhookへの送信はサンドボックスから到達不可のため試みていない(§2-2)。GitHub Actions側の通知は、`8cc26c7`のビルド成功時に(Secretが設定済みであれば)送信されているはず。
