...
最終更新: 2026-09-24(定期実行セッション、v0.57.0リリース: 新規MOB「プリズミウムの使い魔」(Prismium Familiar)を追加。このMODの7体目のMOBにして初のテイム可能な仲間MOBで、`TamableAnimal`をこのMOD初採用した(既存の全MOBは`PathfinderMob`直系)。プリズミウムの欠片を持って右クリックすると1/3の確率(バニラのオウムと同じ仕様)で懐き、以後FollowOwnerGoalでプレイヤーに追従、素手右クリックでSitWhenOrderedToGoalによる待機切り替えができる。飛行AI(FlyingMoveControl+FlyingPathNavigation+WaterAvoidingRandomFlyingGoal)・SquidModelベースの見た目はプリズミウム・ウィスプ(6体目)のものをそのまま流用し、テクスチャーのみHSV色相変換でウィスプの金色系からバラ色〜ラベンダー系に作り直した(`gen_prismium_familiar.py`)。繁殖は`isFood`を常にfalseにして意図的に無効化(`getBreedOffspring`はnullを返す防御的実装)。Prism Realmに重み2の低頻度で自然スポーン(データ駆動、`add_prismium_familiar_spawn_realm.json`)。mobInteract内の`TamableAnimal`各種メソッド・`FollowOwnerGoal`/`SitWhenOrderedToGoal`のコンストラクタ・`AgeableMob#getBreedOffspring`が`Animal`に未実装であること、はすべてmappings.devの1.20.1 mojmapで事前に実在確認済み(未確認Java APIルール順守)。Issue #15/#21は個別ページで再確認したが新規コメント無し(12セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし、新規issue番号の出現も無し。CIのbuild-and-notify(実装コミット647f47f→run 367、バージョンbumpコミットd78e78a→run 368)・Releaseワークフロー(タグv0.57.0→run 70)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=d78e78a...`、鉱石生成検証も引き続き正常であることを確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.57.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
