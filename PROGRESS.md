...
最終更新: 2026-08-31(定期実行セッション、v0.32.0リリース: プリズミウム・コンペンディウム追加、issue #16/#17クローズキュー)
...
1. **【最優先・新規】v0.31.2で「全GUIブロックの画面固まりバグ」を修正した(問題点参照)。修正内容はコードレビューとForge公式ドキュメントの記述に基づく(このサンドボックスでは実機確認不可)。次回、こんぺいとう氏に粉砕機/精錬機/圧縮機だけでなくPrismium Cell/Generator/Pylon/Restorer/WardstoneのGUIも含めて実機確認を依頼すること。直っていなければ最優先で再調査(その場合、今回の仮説=クライアント側Menuコンストラクタが本物のContainerDataを使い回していた、が的外れだった可能性が高いので振り出しに戻ること)。**
2. 【新規・優先度を上げた】GameTestフレームワーク(このMODが既にCuriosAPI検証で使っている`runGameTestServer`基盤)で、ContainerDataの同期(クライアント側`set()`が実際に値を反映するか)を自動検証する仕組みを追加する。今回のバグが8ブロック全てに共通する設計ミスだったにもかかわらずCIビルド成功だけでは検知できなかった教訓を踏まえ、再発防止として優先度を上げた。
3. プリズミウム・クローラー(v0.31.0で追加した5体目のMOB)が実機で意図通り動作しているか(見た目・自然スポーン・鳴き声)、こんぺいとう氏に確認を依頼する。
4. Issue #18対応(Curios統合)の残りの実機確認。**【2026-08-30、こんぺいとう氏が実機確認済み】Guardian CharmはCurios対応済みで、右クリックでcharmスロットに装備できることを確認済み。** 残る未確認は、Featherstone/Emberguard/Vitastone/Magnet Charmの4アイテムのCurios GUIでのスロット表示・ドラッグ&ドロップ・右クリック装備、および全5アイテム共通で「装備中に効果が実際に発動するか」。これらの確認が取れ次第issue #18をクローズする。
5. Issue #21(JEI互換性)への着手方針を検討する。着手する場合は決まり事14(mixin remapRefMap)を思い出すこと。
6. **【2026-08-30、こんぺいとう氏からの指摘で順序変更・記述修正】Prism Realmにまず「平原」の基本地形を追加する。** 現状のPrism Realmは`minecraft:flat`型のチャンクジェネレータによる「水没ワールド」で、バニラのオーバーワールド設定や固定バイオームは使っていない(過去に一度そういう実装をしてこんぺいとう氏から強く指摘され、既に修正済み。決まり事18参照)。専用バイオーム`claudemod:prism_realm`・自作のPerlin/Fractalノイズによるプリズミウムの深層岩/石の境界のまだら化(`PrismiumStoneTransitionFeature`)は実装済みだが、地形の起伏自体が無く地表全体が海になっている(陸地が存在しない)。この土台(陸地)が無いまま巨大山岳地帯のような特徴地形だけを追加すると、「海に急に山岳地帯がある」ような文脈のない不自然な地形になってしまう。
7. 上記6の後に、**各バイオームに固有のボスを伴うダンジョンが低確率で生成される仕組み**を実装する(こんぺいとう氏からの要望、2026-08-30:「平原のボス、山岳地帯のボス、みたいな感じで各バイオームにまれにダンジョンが生成されるようにしてほしい」)。ユーザー直接要望「Prism Realm巨大山岳地帯+ボス」は、この仕組みの山岳バイオーム版として最初に着手する。**6より先に着手しないこと**(海のような場所に唐突に山岳があると意味が分からなくなるため)。規模が大きいため、(a)バイオームごとのダンジョン構造生成、(b)バイオーム固有のボスMOB、(c)低確率でのダンジョン配置ロジック、に分割して段階的に進めること。山岳版が完成したら、平原など他バイオームにも同じ仕組みを横展開する。
8. MOBのカテゴリ拡充を検討する。現状「戦闘」「水中非戦闘」「地上アンビエント」の3種類があり、飛行アンビエントMOB・使い魔的MOB等の新カテゴリを追加する余地がある(バイオーム固有ボスもこの拡充の一環)。
9. 【2026-08-31、新規】v0.32.0で追加したプリズミウム・コンペンディウム(issue #7対応のガイドブック)について、次回こんぺいとう氏に実機確認を依頼すること。確認したい点: (a) 初回ワールド参加時に自動で1冊インベントリに入るか、(b) 本を開いた際に11ページ全て正常に表示され、日本語ページが本のページ内に収まりきっているか(はみ出していないか)、(c) クリエイティブタブにも表示されているか。
10. コンペンディウムを紛失した場合の再入手手段(クラフトレシピ等)がまだ無い。バニラの本+プリズミウムの欠片のような簡単なレシピで再発行できるようにする(専用NBTを持つカスタムレシピが必要、vanillaのBookCloningRecipeのような特殊レシピクラスの実装が要る)。
11. コンペンディウムの内容は現状11ページの概要のみ。今後、各エネルギー機械の詳細な配線図解や、Prism Realmのダンジョン/ボス実装(TODO7)が進んだらその案内ページを追加するなど、実装が進むたびに内容を拡充していく。
