...
最終更新: 2026-09-22(定期実行セッション、v0.55.0リリース: 新規ブロック「蒼白のプリズミウムの壁灯」(Pale Prismium Wall Lamp)を追加。v0.54.0のプリズミウムの壁灯(`PrismiumWallLampBlock`)をそのまま再利用した蒼白ファミリー版で、形状・当たり判定・4方向設置・自動脱落は元と同一、テクスチャー(PALE_*パレットの発光菱形宝石)とレシピ(蒼白のプリズミウムブロックx1+グロウストーンダストx1のシェイプレス、1個産出)のみ差し替え。光レベルは元と同じ13。Pillowで新規生成し16倍拡大プレビューで自己レビュー済み。これで「壁掛け(HORIZONTAL_FACING)」系統も蒼白ファミリー版が揃った。Issue #15/#21は個別ページで再確認したが新規コメント無し(10セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし、新規issue番号の出現も無し。CIのbuild-and-notify(実装コミット82c0aa5→run 361、バージョンbumpコミット5e554cb→run 362)・Releaseワークフロー(タグv0.55.0→run 68)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=5e554cb...`かつ新規レシピのパースエラーが無いことを確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.55.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
...
5. **新装備**: ツール5種・アーマー4種(セット効果: 暗視+水中呼吸)、グラップリングフック、探知アイテム(Locator)、Shield、Bow、Guardian Charm(cheat-death)、Featherstone/Emberguard/Vitastone/Magnet Charm/Aegis Charm/Frostguard Charm(完全パッシブ系、v0.53.0で6種目・凍結ダメージ軽減が追加)まで実装済み。一部はCuriosAPI対応済み(Issue #18)。
