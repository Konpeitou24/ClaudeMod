...
最終更新: 2026-09-19(定期実行セッション、v0.52.0リリース: 新規アイテム「プリズミウムの護盾石」(Prismium Aegis Charm)を追加。羽石(Featherstone)/火除け石(Emberguard)/活力石(Vitastone)/磁石の護符(Magnet Charm)に続く5つ目の完全パッシブ系charm(装備不要、インベントリに持っているだけで効果、Curios charmスロットにも対応)。効果は爆発ダメージ50%軽減。既存の羽石/火除け石/活力石が落下・炎・回復をカバーしていた一方、警戒石(Wardstone)や将来のPrism Realmダンジョンボス構想が出しうる爆発ダメージには対応するものが無かった穴を埋めた。新しいJava API`DamageTypeTags.IS_EXPLOSION`はmappings.devで実在を事前確認してから使用(未確認API使用ルールを遵守)。テクスチャーは既存3種と同じ「石+対角線モチーフ+中央のプリズミウム宝石」構図を踏襲しつつ、モチーフを盾に、配色を未使用だった紫(バイオレット)にしてPillowで新規生成、24倍プレビューで自己レビュー済み。クラフトレシピは黒曜石x2+火薬x2+プリズミウムの欠片x1の十字配置shaped。Issue #15/#21は個別ページで再確認したが新規コメント無し(7セッション連続で変化無し)、Issues一覧もOpen 2のまま変化なし。CIのbuild-and-notify(コード変更コミットace2871→run 351、バージョンbumpコミットdbcfe40→run 352)・Releaseワークフロー(run 65)とも実際にStatus Successをブラウザで確認、`builds/last_datapack_validation_summary.txt`でも`status=ok commit=dbcfe40...`を確認、`https://github.com/Konpeitou24/ClaudeMod/releases/tag/v0.52.0`をブラウザで開きAssets 3付きで公開されていることも確認済み。詳細はTODO・問題点・その他を参照)
...
5. **新装備**: ツール5種・アーマー4種(セット効果: 暗視+水中呼吸)、グラップリングフック、探知アイテム(Locator)、Shield、Bow、Guardian Charm(cheat-death)、Featherstone/Emberguard/Vitastone/Magnet Charm/Aegis Charm(完全パッシブ系、v0.52.0で5種目・爆発ダメージ軽減が追加)まで実装済み。一部はCuriosAPI対応済み(Issue #18)。
