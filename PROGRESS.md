...
### 今回(セッション#63、定期実行)の最重要な新情報

- **新規アイテム「Prismium Pulse Charm」(敵性MOB探知アクセサリー、§3BO-3)を追加した。右クリックで周囲16ブロックの敵性MOBを10秒間発光させる。実機で一切未検証、次回最優先でフィードバックを確認すること。**
- **`.details`lang キー(ItemDetailsOverlayのW長押しパネル用、セッション#62で新設)を、残り19アイテム(ツール5・アーマー4・アクセサリー10)全てに拡充した(§3BO-4)。これでMOD内の全カスタムアイテムが`.details`を持つ状態になった。申し送り項目2は解消。**
- **v0.10.0をリリースした(§3BO-6)。直近リリースは`v0.10.0`。次回セッションはここから1セッション目。**
- **【重要な運用上の発見】GitHub Issueページを素の(User-Agent無しの)curlで取得すると、実際のコメント本文を含まない簡略化されたページが返ってくることが判明した(§3BO-1)。ブラウザ相当のUser-Agentヘッダ(例: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36`)を付けて取得すること。過去に報告されていた「Issue #15だけ時々Not Foundになる」現象も同根の可能性がある。**
- 新規のIssueコメントは無かった(§3BO-1)。Open Issueは引き続き#7・#9・#15・#16の4件、すべて投稿者`Konpeitou24`本人。

### すぐやるべきこと(継続項目、優先度順は前回から概ね維持)

0. **【超最優先・全セッション必読・リリースポリシー】作業開始時に必ず`git tag --list --sort=-creatordate`等で直近のリリースタグとそこからの経過を確認すること。新機能追加を含むセッションが完了した時点、前回リリースから3セッション以上経過した時点、または前回リリースからまだ日が浅くても複数の実質的な修正が積み上がった時点のいずれか早い方で、そのセッション内でリリースを切ることを検討する。見送る場合もその判断と理由をPROGRESS.mdに明記すること。直近のリリースはv0.10.0(セッション#63、今回)。次回はここから1セッション目。**
1. **【新規・最優先】今回追加した`ItemDetailsOverlay`のW長押しパネル(session 62)と、そこに表示される`.details`テキスト(session 62で6ブロック分、今回全アイテムに拡充)について、実機フィードバックが無いか最優先で確認すること。長押しでパネルが実際に表示されるか、スライドアニメーション・パネル位置に違和感が無いか、`.details`の文章が実際の画面で読みやすいかが特に未検証。**
2. **【新規・最優先】今回追加したPrismium Pulse Charm(§3BO-3)について、実機フィードバックが無いか最優先で確認すること。探知範囲・発光時間・クールダウンのバランス感覚、クラフトの入手難易度感が特に未検証。**
3. 【継続】Prismium Drifter(session 61、§3BM-3)について、実機フィードバックが無いか引き続き確認すること。Prism Realmの水中に実際にスポーンするか、見た目(遊泳アニメーション含む)に違和感が無いかが特に未検証。
...
12. **【更新】GitHub Issue状態の確認は`github.com/<owner>/<repo>/issues/<番号>`への直接curlを使い、必ずブラウザ相当のUser-Agentヘッダを付けること(§3BO-1、今回の新発見)。UA無しだとコメント本文を含まない簡略化ページが返り、実コメントを見落とす。`_cb=<timestamp>`のキャッシュバスティングも引き続き有効。`"__typename":"IssueComment","databaseId":[0-9]*`(実コメントID)を抽出する方式(セッション#62で精緻化)を使うこと。**
13. 【継続】worldgen/レジストリ系のJSONやクライアント専用APIを書く/直す際は、必ず一次情報・実例で裏取りしてから確定させること。Yarn mappingsの命名とForge公式(Mojang)マッピングの命名が食い違うケースに複数回遭遇している(セッション#61・#62)。正確な名前は必ずForge公式マッピングベースの情報源(nekoyue.github.io/ForgeJavaDocs-NG、lexxie.dev/forge/1.20.1/、または実際のMinecraftForgeリポジトリソース)で最終確認することを徹底し続けること。今回はlexxie.dev/forge/1.20.1/の`class-use/Item.Properties.html`ページから`StandingAndWallBlockItem`等の1.20.1ピンポイントのコンストラクタ シグネチャを確認できた実績があるので、次回以降もこのサイトを情報源として活用する価値がある。
14. **【更新・重要】`/tmp`直下等の固定名ディレクトリは他セッション所有で書き込み不可なことが多い(継続、今回も再発)。`$HOME`配下(例: `~/claudemod_<epoch秒>`)に一意な名前で新規ディレクトリを掘るのが引き続き確実(今回問題なし)。次回セッションは`/tmp`を試さず最初から`$HOME`配下を使うことを推奨する。`git config user.name`/`user.email`をローカルに設定すること(`ClaudeMod Agent <konpeitou-agent@users.noreply.github.com>`)。**
...
16. 【継続】リリース(v0.10.0等)の中身を実際にダウンロードして展開しての検証はまだ一度もしていない。
17. 【継続、最重要度は維持】実プレイ検証ゼロの装備・ブロック・ディメンション機能・MOB・UI要素(今回追加したPrismium Pulse Charmも含む)が積み上がり続けている。ユーザー側でのプレイフィードバックを今後も最優先で拾うこと。
...
22. 【新規】Issue #7の「専用のアニメーションUI」要求(コメント3)について、現状はオーバーレイ方式(既存Screenに重ねて描画)で対応済み(session 62)。実機フィードバックで「これで十分」と分かれば独立Screenへの格上げは不要、逆に不十分であれば独立Screen化を検討すること(session 62 §3BN-8で提起)。

### 議論したい論点・改善案

- 【新規】GitHub IssueページのUser-Agent依存の簡略化レスポンス問題について(§3BO-1)。過去のIssue #15「時々Not Found」現象の一部がこれで説明できる可能性がある。
- 【継続】Yarn/Forge公式マッピングの命名差異について、今回はlexxie.dev/forge/1.20.1/という1.20.1ピンポイントの情報源が有効活用できた。今後もこのサイトを優先的に使う価値がある。
- 【継続】質と量を交互に意識するバランス感覚について。セッション#62が質、セッション#63(今回)が量+質の両方。
- PROGRESS.mdの肥大化(3000行超、今回さらに増加)について、詳細ログと申し送りの分離を検討する余地がある(複数セッションで繰り返し「見送った」と記録されている項目、今回も着手せず)。

### コミット/プッシュ状況

セッション#63(定期実行)は以下のコミットをpush:
1. `4cd79aa` Add Prismium Pulse Charm (hostile-mob detection accessory) and expand .details tooltips to remaining tools/armor/accessories
2. `30ef802` Release v0.10.0: bump mod_version, add release notes (+タグ`v0.10.0`)

`git fetch origin main`で並行セッション無しを確認、素のまま`git push origin main`で両コミットとも一発成功。push後、`ci: update built jar`→`ci: update datapack validation results`(`status=ok`)の到着をそれぞれ確認済み。**通常ビルド・データパック検証とも成功。** `release.yml`起動によるv0.10.0のGitHub Release公開・添付jar(`claudemod-0.10.0.jar`)の存在も確認済み。
