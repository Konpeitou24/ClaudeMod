...
15. 【セッション#9で新規発覚】Prismium Generator(§3G)は以下すべて未検証・既知の割り切り:
    ...
    - Generatorも(Prismium Cellと同様)ブロックを壊すと燃焼時間・蓄積FEの両方が失われる(§4-13のCellの制約と同じ理由、同じ未対応)。
...
18. 【セッション#10で新規発覚】Prismium Cable(§3I)は以下すべて未検証・既知の割り切り:
    ...
    - Cell/Generatorと同様、ブロックを壊すとバッファ中のFEは失われる(§4-13/§4-15と同じ理由、同じ未対応)。ただしCableは元々バッファが小さく「通過するだけ」の設計なので、この制約による実害はCell/Generatorほど大きくないと考えられる。
...
### すぐやるべきこと
1. セッション開始時のビルド確認は、引き続き**`git fetch origin main`で`ci: update built jar`コミットが自分の知っている最新コミットの直後に付いているかを見る方法を第一手段にすること**(session 8で確立、session 9・10でも問題なく機能した)。このセッション終了時点でのmain最新コミットは `c422c13`(`ci: update built jar`、Prismium Cable追加コミット`b7e5eb4`の直後)。次回はここからの差分を見ればよい。
2. 【継続、§0-2】セッション開始時に必ずGitHub Open Issueを確認すること。今回確認した時点ではissue #1(顔が見えない、session 9で対応済み)のみがOpenのまま残っている。ユーザー側でのクローズ待ちなので、次回以降も新規Issueが無いか確認しつつ、#1がクローズされたかどうかも一応見ておくとよい(見ても対応することは無いが、状況把握のため)。
3. push前に必ず `git fetch origin main` → 差分があれば `git rebase origin/main`(§2-5)。セッション#10でも他セッションとの並行は検知しなかったが、毎回確認すること。
4. **push後は必ず`git fetch`ポーリングでビルド結果を確認し、failedならそのセッション内で追いかけて直す**。「pushして終わり」にしない。
5. 【新規、優先度高】Prismium Cable(§3I)は、Generator・Cellと合わせて「発電→送電(直接 or 配線経由)→蓄電」の3点セットが揃ったが、**実際にゲーム内でGenerator→Cable→Cellと並べて動作確認したセッションはまだゼロ**。次にできることは主に2つ: (a) さらにコードレビュー可能な粒度でロジックを分解・整理する、(b) ユーザー側でプレイテストする手段があれば、その結果をPROGRESS.mdに反映してもらう(§「議論したい論点」参照)。
6. 【継続、優先度中、3ブロックに拡大】Cell・Generator・Cableともブロックを壊すとFE(と燃焼時間)が失われる問題(§4-13、§4-15、§4-18)がまだ未対応のまま3つの機械に蓄積している。そろそろ`loot_table`の`copy_nbt`関数、または`BlockItem`側でNBTを持たせる対応に着手する価値が高まってきた(Cableは実害が小さいが、Cell/Generatorは依然として重要)。
7. 【新規、優先度中〜低】Prismium Cableの見た目は「常に同じ中央キューブ」で、柵/ガラス板のようなマルチパートblockstateによる接続表現(方向ごとのboolean属性)が無い。次にケーブルを大量に敷設する場面が来る前に、この磨き込みに着手する価値がある(§3I-1の「既知の割り切り」参照)。実装難度はこのMOD内でこれまでで最も高いモデリング作業になる見込み(多方向の当たり判定・レンダリング条件分岐)なので、まとまった時間が取れるセッションで着手するのが望ましい。
8. 【継続】Prismiumグラップリングフック・ツール5種のギミック・アーマーセット効果・ワールド生成・アーマー装着時テクスチャー・Prismium Coreのタグ挙動は、いずれも実プレイでしか確認できない「未検証」のまま。今後このサンドボックス以外でプレイテストする手段があれば、それを反映する形でPROGRESS.mdを更新できると精度が上がる。
9. **ロードマップの2大柱(§1)のうち、Prismium Energyは(発電→配線→蓄電まで一通り繋がったことで)大きく前進した。Prism Realmディメンションはセッション#1から依然として未着手のまま**。もしまとまった時間が取れるセッションがあれば、「空のディメンション定義+ポータル枠ブロックのみ、地形はバニラ流用」のような最小の第一歩を検討する価値がある。
...
- **プレイテストの手段が無い問題**: このサンドボックスは`./gradlew build`はおろかMinecraftそのものを起動できないため、「コンパイルが通る」以上の検証が原理的に一切できない。Prismium Cell・Generator・Cableのような複数ブロックが連携する機構(capabilityの実際のやり取り)は特にこの制約の影響を強く受ける。もしユーザー側で時々実際にビルド済みjar(`builds/ClaudeMod-latest.jar`)を導入してプレイし、フィードバックをPROGRESS.mdか何らかの形で次回セッションに伝えられる仕組みがあれば、コード品質のボトルネックが「コンパイルが通るか」から「実際に遊んで楽しいか」に移行できる。これは継続的な相談事項として残しておく。
- **エネルギーシステムの設計方針の続き**: Prismium Cell(容量100,000 FE)・Generator(バッファ8,000 FE、送電200 FE/tick)・Cable(容量400 FE、転送400 FE/tick)の数値関係は、session 10時点での初期見積もりの叩き台。実際に3つを組み合わせてボトルネックが生じないか(特にCableの400 FE/tickがGenerator出力とCell受電のどちらとも十分な余裕があるか)は次回以降、より多くのケーブル・複数台の機械を組み合わせる場面で再検討したい。
- **Prismium Cableの接続表現**: §5-7参照。柵/ガラス板方式のマルチパートモデルに発展させるかどうか、あるいは「常に同じキューブ」のシンプルな見た目のまま行くかは、まだ方針が固まっていない論点。
- **Prism Realm ディメンションの雰囲気**: 縦方向の探索(空中島、深い縦穴)や視認性の良いランドマーク配置を検討したい。まだ未着手。Prismium Lantern・グラップリングフック・Prismium Cell/Generator/Cableはこのディメンションでの「持ち込み道具・据え置き設備」としても機能しそうなので、着手時に接続できるとよい。
- **アーマーの見た目**: 現状は「灰色フレーム+クリスタル」で統一したが、4部位が実際に体に乗った状態でどう見えるかは未確認。プレイ確認できたら、必要なら差し替える。
- **テクスチャー生成の再利用可能な知見**: 「意図的な透明の隙間は輪郭幅×2より広く取る」「新しい視覚言語を足すときも密度は控えめに」「塗り潰した面から穴をくり抜く方が小キャンバスでは安全」「既存パレットを新しいテクスチャーに流用する時はPRISMIUM_OUTLINEとのコントラストを個別確認」「同じファミリーの2つ目以降のブロックは初代で検証済みのパレットをそのまま再利用するとリスクを回避できる」(session 9)に加え、**session 10では「非フルキューブの自作モデルでは`cullface`をブロック境界に接していない面に付けない(接していない面に付けると隣接ブロックの状態次第で本来見えるべき面が誤って消える)」という新しい教訓が加わった(§3I-1)**。`scripts/textures/common.py`のような共通ユーティリティ+コメント集にまとめる案は依然未着手。
- **CIのRun 4 failed の原因調査**(セッション#3から継続)。引き続き優先度は低い。

### コミット/プッシュ状況
このセッションの変更は1つのコミット: `Add Prismium Cable: relay block connecting Generator/Cell across a gap`(`b7e5eb4`)。`git fetch`では他セッションとの衝突は無く、一度のpushでそのまま成功(プロキシ変数はいじらず、これで7セッション連続無改変成功)。push後、`git fetch`ポーリング(約15秒間隔・8回・計約120秒)で`ci: update built jar [skip ci]`コミット(`c422c13`、jarサイズが71,837→79,251バイトに増加)の到着を確認し、ビルド成功を実証した。つまりこのセッションは、Prismium Energyの3つ目のピース(Generator/Cellを離れた場所でもつなげる配線)を、実装(MOD初の非フルキューブブロック・2例目のBlockEntityTicker・共通ヘルパーへのリファクタリング)→テクスチャー自作(描き直し無し)→push→ビルド成功確認まで一貫して完結できたセッションだった。GitHub Issueも確認し、新規のOpenは無いことを確認済み(#1は対応済みでクローズ待ちのまま)。
