...
3. **新ディメンション**: 「Prism Realm」。データパック駆動のディメンション、テレポート用アイテム(Prismium Rift Shard)まで実装済み。専用地形・専用鉱石・専用バイオーム・本格的なポータルブロックはまだ無い。将来的には各バイオームに固有ボスを伴うダンジョンがまれに生成される仕組みを構想中(山岳地帯版から着手予定、TODO参照)。
