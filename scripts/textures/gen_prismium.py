...
block/prismium_block.png is a special case (session 37, interactive
follow-up): the user hand-drew their own Prismium Block texture and asked
for it to be used directly rather than regenerated by make_prismium_block()
below. That function is kept (both as a fallback and because its diagonal-
band gradient technique is reused by gen_prismium_core.py etc.), but this
script's __main__ no longer calls save() on its output - instead it copies
the user's art (scripts/textures/reference/user_submitted_prismium_block_
2026-08-18.png) straight into the assets folder. See PROGRESS.md session 37
follow-up for the full story, including how the PRISMIUM_* palette below was
re-derived from this same image so every other block/item in the mod shares
its colors.

All other output is deterministic (fixed RNG seed) so re-running regenerates
the same art. Run from repo root: python3 scripts/textures/gen_prismium.py
"""
import random
...
# Prismium crystal palette: teal/cyan primary with a magenta energy fleck,
# kept distinct from vanilla diamond (blue-white) and emerald (green).
#
# UPDATED session 37 (interactive follow-up): re-derived from the user's
# own hand-drawn Prismium Block texture (scripts/textures/reference/
# user_submitted_prismium_block_2026-08-18.png), sampled directly from its
# pixel grid rather than eyeballed - OUTLINE/SHADOW from its bottom-right
# shading corner, BASE from its border ring, MID/HILITE from its two most
# common interior tones, ACCENT/ACCENT_DARK from its four magenta gem
# clusters. The previous palette (session 1: #0B3D3C/#1E7A78/#3FBDB8/
# #66D9D2/#B9FFF3/#C97BFF/#7A3FA6) is still visible in git history if a
# future session needs to compare. This is the single palette definition
# every other gen_prismium_*.py script copies verbatim (grep for
# 'PRISMIUM_OUTLINE = "#' across scripts/textures/ to find them all) - all
# of them were updated together with this file so the whole mod shares one
# palette, matching the user's request to use their art "as the basis for
# every block".
...
PRISMIUM_ACCENT = "#FF7CFC"  # magenta energy fleck/gem, ties into future energy system
...
def use_user_submitted_block_texture():
    """Copy the user's hand-drawn Prismium Block art in as the real asset.

    Session 37 (interactive follow-up): the user made their own 16x16
    texture and asked for it to be used directly rather than regenerated
    by make_prismium_block(). Converts to RGBA (the source PNG is plain
    RGB) for consistency with every other texture in the mod, but does
    not otherwise touch a single pixel.
    """
    src = REPO_ROOT / "scripts/textures/reference/user_submitted_prismium_block_2026-08-18.png"
    img = Image.open(src).convert("RGBA")
    assert img.size == (SIZE, SIZE), f"expected {SIZE}x{SIZE}, got {img.size}"
    out = ASSETS / "block/prismium_block.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    img.save(out)
    print(f"wrote {out.relative_to(REPO_ROOT)} (from user-submitted art, not make_prismium_block())")
...
if __name__ == "__main__":
    ...
    use_user_submitted_block_texture()
