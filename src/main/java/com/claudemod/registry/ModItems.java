...
import com.claudemod.item.PrismiumAntivenomCharmItem;
...
public class ModItems {
    ...
    // Pale Prismium Block building variety (session 78). See ModBlocks.
    // PALE_PRISMIUM_BLOCK_SLAB / WALL / STAIRS.
    public static final RegistryObject<Item> PALE_PRISMIUM_BLOCK_SLAB_ITEM = ITEMS.register("pale_prismium_block_slab",
            () -> new BlockItem(ModBlocks.PALE_PRISMIUM_BLOCK_SLAB.get(), new Item.Properties()));

    public static final RegistryObject<Item> PALE_PRISMIUM_BLOCK_WALL_ITEM = ITEMS.register("pale_prismium_block_wall",
            () -> new BlockItem(ModBlocks.PALE_PRISMIUM_BLOCK_WALL.get(), new Item.Properties()));
    ...
    // Seventh mob spawn egg (see PrismiumFamiliarEntity's javadoc). Soft
    // rose/lavender colors distinguish it from Wisp's warm gold/violet -
    // a "friendly companion" palette rather than a "background ambient
    // light" one, matching the reskin in gen_prismium_familiar.py.
    public static final RegistryObject<Item> PRISMIUM_FAMILIAR_SPAWN_EGG = ITEMS.register("prismium_familiar_spawn_egg",
            () -> new ForgeSpawnEggItem(ModEntities.PRISMIUM_FAMILIAR, 0xf7c6e0, 0x6a3b7a, new Item.Properties()));
    ...
    // Pale Prismium Geode Cluster (scheduled session, 2026-09-15):
    // BlockItem for the Pale Prismium family's own crystal surface
    // decoration - see ModBlocks.PALE_PRISMIUM_GEODE_CLUSTER.
    ...
    // Prismium Frostguard Charm (scheduled session, 2026-09-20): the
    // mod's sixth purely passive accessory - see
    // PrismiumFrostguardCharmItem / PrismiumFrostguardCharmHandler.
    // Same "stacks normally, presence-only" treatment as
    // Featherstone/Emberguard/Vitastone/Magnet Charm/Aegis Charm.
    public static final RegistryObject<Item> PRISMIUM_FROSTGUARD_CHARM = ITEMS.register("prismium_frostguard_charm",
            () -> new PrismiumFrostguardCharmItem(new Item.Properties()));

    // Prismium Antivenom Charm (scheduled session, 2026-09-23): the
    // mod's seventh purely passive accessory - see
    // PrismiumAntivenomCharmItem / PrismiumAntivenomCharmHandler.
    // Same "stacks normally, presence-only" treatment as
    // Featherstone/Emberguard/Vitastone/Magnet Charm/Aegis Charm/
    // Frostguard Charm.
    public static final RegistryObject<Item> PRISMIUM_ANTIVENOM_CHARM = ITEMS.register("prismium_antivenom_charm",
            () -> new PrismiumAntivenomCharmItem(new Item.Properties()));
