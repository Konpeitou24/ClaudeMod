...
/**
 * Screen for Prismium Compressor's GUI (session 70, scheduled) - see
 * {@link PrismiumCompressorMenu} for the slot layout/shift-click logic.
 * Structurally a direct copy of {@link PrismiumSmelterScreen} (same
 * 176x148 panel, same progress/energy bar geometry) with one palette
 * change: the progress fill uses a cool steel-blue/white gradient
 * (matching Prismium Alloy Ingot's own item-icon palette) instead of
 * Smelter's amber or Pulverizer's magenta, so all three "twin" GUIs
 * still read as visually distinct machines at a glance.
 */
public class PrismiumCompressorScreen extends AbstractContainerScreen<PrismiumCompressorMenu> {

    private static final ResourceLocation TEXTURE =
            new ResourceLocation(ClaudeMod.MOD_ID, "textures/gui/container/prismium_compressor.png");
    ...
    // Cool steel-blue/white progress fill - matches Prismium Alloy
    // Ingot's own item-icon metal palette (see
    // gen_prismium_alloy_ingot.py's ALLOY_BASE/ALLOY_HILITE), distinct
    // from Smelter's amber and Pulverizer's magenta so all three GUIs
    // stay visually distinguishable despite sharing a layout.
    private static final int PROGRESS_BASE = 0xFF6E8A9E;
    private static final int PROGRESS_HILITE = 0xFFE4F1F7;

    private static final int STATUS_ACTIVE = 0xFF4CD97B;
    private static final int STATUS_IDLE = 0xFF8A8A8A;

    public PrismiumCompressorScreen(PrismiumCompressorMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        Component statusText = Component.translatable(menu.isActive()
                ? "gui.claudemod.compressor_status_active"
                : "gui.claudemod.compressor_status_idle");
